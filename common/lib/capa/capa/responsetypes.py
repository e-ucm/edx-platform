...
#-----------------------------------------------------------------------------
@registry.register
class MiniGameResponse(LoncapaResponse):
    """
    Custom response.  The python code to be run should be in <answer>...</answer>
    or in a <script>...</script>
    """

    tags = ['minigameresponse']

    allowed_inputfields = ['jsinput']
    code = None
    expect = None
	...
	message=""
        ...
        for k in range(len(correct)):
            npoints += 1 if correct[k] == 'correct' else 0

        for k in range(len(messages)):
            message+=messages[k]
	

        correct_map.set(idset[0], correct[0], msg=message,
                            npoints=npoints)



        overall_message = self.clean_message_html("tu puntuacion es: "+ str(npoints)+"/6")
        correct_map.set_overall_message(overall_message)
        return correct_map
    ...
    def execute_check_function(self, idset, submission):
        ...
        if isinstance(self.code, basestring):
            ...
            if isinstance(ret, dict):
                ...
                if 'imagenes' in ret:
                    #overall_message = ret.get('overall_message', '')
                    imagen_list = ret['imagenes']
                    messages_list = ret['mensajes']

                    correct = []
                    messages = []
                    for imagen_dict in imagen_list:
                        correct.append('correct'
                                       if imagen_dict['ok'] else 'incorrect')
                    for messages_dict in messages_list:
                        msg = (self.clean_message_html(messages_dict['msg'])
                               if 'msg' in messages_dict else None)
                        messages.append(msg)
                    tiempo=ret['tiempo']
                    tiempomili=tiempo['tiemp']
                    tiempomin=(tiempomili/60000)
                    tiemposeg= ((tiempomili % 60000)/1000)
                    messages.append("EL tiempo del ejercicio ha sido "+ str(tiempomin)+" minutos y " + str(tiemposeg)+" segundos.")
                    self.context['messages'] = messages
                    #self.context['overall_message'] = overall_message
